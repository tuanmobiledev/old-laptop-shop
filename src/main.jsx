...
function ProductDetailPage({ lang, onClose, product, productList, setProduct, t }) {
  ...
  const toggleAddon = (addon) => setSelectedAddons((current) => {
    const singleChoice = addon.type === 'warranty' || addon.sku.startsWith('RAM-') || addon.sku.startsWith('SSD-');
    const base = singleChoice ? current.filter((id) => {
      const item = addons.find((entry) => entry.wooId === id);
      return item && item.type !== addon.type && !(item.sku.startsWith('RAM-') && addon.sku.startsWith('RAM-')) && !(item.sku.startsWith('SSD-') && addon.sku.startsWith('SSD-'));
    }) : current;
    return base.includes(addon.wooId) ? base.filter((id) => id !== addon.wooId) : [...base, addon.wooId];
  });
  ...
  const configurationText = [`${product.name} - ${formatCurrency(displayProduct.price)}`, ...chosenAddons.map((addon) => `${addon.name}: ${formatCurrency(addon.price)}`), `Tạm tính: ${formatCurrency(orderTotal)}`, shareUrl].join('\n');
  ...
  const submitOrder = async (event) => {
    ...
    try {
      const restBase = window.OSCAR_WP?.restUrl || '/wp-json/oscar/v1/';
      const response = await fetch(`${restBase}orders`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: orderForm.name, phone: orderForm.phone, note: orderForm.note, productIds: [product.wooId, ...selectedAddons] }) });
