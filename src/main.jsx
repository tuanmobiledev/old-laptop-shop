...
const productImageFallback = (event) => {
  ...
  const fallback = products.find((product) => product.name === img.alt)?.image || '/oscar-cover.jpg';
...
function ProductDetailPage({ lang, onClose, product, productList, setProduct, t }) {
  ...
  useEffect(() => {
    ...
    setActiveImage(normalizeImagePath(product.image));
